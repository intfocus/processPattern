...
int process(char szParam[MAX_INPUT_LEN]){
    ...
    char szInput[MAX_INPUT_LEN], szTemp[MAX_INPUT_LEN], szChStart[MAX_INPUT_LEN], szChEnd[MAX_INPUT_LEN];
    int nTemp=0,nLastDigit=0,nLastQuant=0,nZeroFlag=0,nLastIsTen=0,sznTemp=0;
...
step_4_4_1: // #001 加入 step_4_4_1, 解决 【一百五十万】的问题         
    ...
    debug_printf("pChStart=[%s],-6=[%s],-5=[%s], pch3=[%s]\n",pChStart,pChStart-6,pChStart-5, szChEnd);
